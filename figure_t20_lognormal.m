...
for atl = [2] %1:20
    ...
    for iM = [1 5] %1:length(metrics) % [1 5] %
        ...
        Ca_humNoCt = load(sprintf('%s/xsub_out_all_%i_atl%i_noCt.mat',dir_cacheL,iM,atl));
        ...
        AdjMagNoThresh = nan(n_rois,n_rois);
        AdjMagNoThreshCond = false(n_rois,n_rois);
        ...
        for i1 = 1:n_rois
            for i2 = 1:n_rois
                ...
                AANoCt = Ca_humNoCt.AdjAtl{i1,i2};
                ...
                if ( (~ isempty(AA))  && (n_pairs >= n_pairs_thresh) && (n_subs >= n_subs_thresh))
                    ...
                    % No threshold coherence
                    if ( ~isempty(AANoCt(AANoCt ~= 0)) )
                        AdjMagNoThresh(i1,i2) = nanmean(AANoCt(AANoCt ~= 0));
                    else
                        AdjMagNoThresh(i1,i2) = 0;
                    end
...
%         return
        ...
        for i1 = 1:(n_rois-1)
            for i2 = (i1+1):n_rois
                if ((i1 ~= 1) && (i2 ~= 1))
                    ...
                    AdjVv = [AdjVv; L4];
                    AdjVctv = [AdjVctv; L5];
                    AdjVNoCt = [AdjVNoCt; AdjMagNoThresh(i1,i2)];
                    AdjVNoCtCond = [AdjVNoCtCond; AdjMagNoThreshCond(i1,i2)];
                end
            end
        end
        ...
        AdjVv = AdjVv(AdjV>0);
        AdjV = AdjV(AdjV>0);
        AdjVNoCtBelow = AdjVNoCt(AdjVNoCtCond ~= 1);
        AdjVNoCtBelow = AdjVNoCtBelow(~isnan(AdjVNoCtBelow));
        ...
        box off;
        set(gca,'TickDir','out');
        print(h,sprintf('figures/T20_lognormal/metric%i_atl%i_mag_vs_ct_r-%i_p-%i',iM,atl,round(r*1000),round(pval*1000)),'-depsc');
        print(h,sprintf('figures/T20_lognormal/metric%i_atl%i_mag_vs_ct_r-%i_p-%i',iM,atl,round(r*1000),round(pval*1000)),'-dpng');
        close(h);
        ...
        %return
        print(hh,sprintf('figures/T20_lognormal/metric%i_atl%i_hist',iM,atl),'-dsvg');
        ...
        % --- KS test ---
        fprintf('[*] Below threshold gauss fits\n');
        Xt = (log10(AdjVNoCtBelow));
        [h,p,ksstat,cv] = kstest((Xt-mean(Xt))/std(Xt));
        fprintf('[*] Log(C) kstest p=%.4d, n=%i\n',p,length(Xt));
        fprintf('\tmin:%.8f, max:%.8f\n',min(Xt),max(Xt));
        % --- Linear test ---
...
%                     end
                ...
                print(h,sprintf('figures/T20_lognormal/metric%i_atl%i_%s',iM,atl,rois{ir}),'-depsc');
                print(h,sprintf('figures/T20_lognormal/metric%i_atl%i_%s',iM,atl,rois{ir}),'-dpng');
                close(h);
...
%     plot(x_cen,y_norm2,':','color',cc(1,:));
    ...
    % [*] Log(C) kstest p=1.4780e-01, n=183
    %return
    print(hh,sprintf('figures/T20_lognormal/150_metric%i_atl%i_hist',iM,atl),'-dsvg');
    %saveas(gcf,sprintf('figures/T20_lognormal/metric%i_atl%i_hist',iM,atl),'epsc');
    close(hh);
end
...
% figure_t20_lognormal
% mkdir: cannot create directory ‘figures’: File exists
% mkdir: cannot create directory ‘figures/T20_lognormal’: File exists
% [*] Log(C) kstest p=5.9134e-01, n=193
% 	min:-0.83778956, max:-0.24939548
% [*] Linear kstest p=4.0133e-02, n=193
% 	min:0.14528154, max:0.56312463
% [caudalmiddlefrontal] n_bip_pairs: 307
% [fusiform] n_bip_pairs: 1774
% [inferiorparietal] n_bip_pairs: 758
% [inferiortemporal] n_bip_pairs: 3046
% [lateralorbitofrontal] n_bip_pairs: 755
% [middletemporal] n_bip_pairs: 3005
% [parsopercularis] n_bip_pairs: 438
% [parsorbitalis] n_bip_pairs: 673
% [parstriangularis] n_bip_pairs: 605
% [postcentral] n_bip_pairs: 1003
% [precentral] n_bip_pairs: 922
% [rostralmiddlefrontal] n_bip_pairs: 877
% [superiortemporal] n_bip_pairs: 1685
% [supramarginal] n_bip_pairs: 1140
% [temporalpole] n_bip_pairs: 321
% [*] Log(C) kstest p=1.2432e-01, n=184
% 	min:-0.79689819, max:-0.24476117
% [*] Linear kstest p=5.1673e-02, n=184
% 	min:0.15962533, max:0.56916584
% [fusiform] n_bip_pairs: 1145
% [inferiorparietal] n_bip_pairs: 501
% [inferiortemporal] n_bip_pairs: 2317
% [lateralorbitofrontal] n_bip_pairs: 691
% [middletemporal] n_bip_pairs: 2369
% [parsopercularis] n_bip_pairs: 394
% [parsorbitalis] n_bip_pairs: 556
% [parstriangularis] n_bip_pairs: 528
% [postcentral] n_bip_pairs: 691
% [precentral] n_bip_pairs: 777
% [rostralmiddlefrontal] n_bip_pairs: 491
% [superiortemporal] n_bip_pairs: 1415
% [supramarginal] n_bip_pairs: 771
% [temporalpole] n_bip_pairs: 309
...
% [*] Log(C) kstest p=1.8247e-29, n=2110
% 	min:-0.94022935, max:-0.10760925
% [*] Linear kstest p=6.7760e-43, n=2110
% 	min:0.11475474, max:0.78053206