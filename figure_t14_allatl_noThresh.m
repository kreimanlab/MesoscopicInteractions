...
system('mkdir figures/T14_allatl_noThresh');
...
for atl = 2 %1:20 %1:20
    ...
    system(sprintf('mkdir figures/T14_allatl_noThresh/atl%i_%s',atl,AtlNames{atl}));
...
%adjct_dist_cl = adjct_dist(ind_hum2mac,ind_hum2mac);
            ...
            %                 yc = yc + 1;
                    ...
                    save(sprintf('./cache/figure_t14_noThresh_%i_atl%i_%s',iM,atl,AtlNames{atl}),'AdjMag','AdjMagVar','Adj_plt','Adj_plt2','Adj_plt2_cl','Y','Z','M','cluster_i','rois_plt','atl');
...
%                 end
    ...
    %             xtickangle(90);
        ...
        %         end
                ...
                %
                print(h,sprintf('figures/T14_allatl_noThresh/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss),fig_fmt);
                if (trig_eps)
                    print(h,sprintf('figures/T14_allatl_noThresh/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss),'-depsc');
                end
...
%                     end
%                     axis off;
%                     print(h,sprintf('figures/T14_allatl_noThresh/atl%i_%s/Adj_%s_%s_dendro',atl,AtlNames{atl},metric,ss),fig_fmt);
%                     if (trig_eps)
%                         print(h,sprintf('figures/T14_allatl_noThresh/atl%i_%s/Adj_%s_%s_dendro',atl,AtlNames{atl},metric,ss),'-depsc');
%                     end
%                     close(h);
%                 end
                ...
                if (iii2 == 1)
                    ...
                    fprintf('[*] saved file: %s\n',sprintf('figures/T14_allatl_noThresh/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss));
...
%                 fprintf('[*] Coherence Stdev n: %.2f\n',numel(V_std));
    ...
    %             axis tight;
                ...
                ofname = sprintf('figures/T14_allatl_noThresh/atl%i_%s/Npairs_%s_mag_%i_p_%d_cp_%i_p_%d',atl,AtlNames{atl},metric,...
    ...
    %             axis tight;
                ...
                ofname = sprintf('figures/T14_allatl_noThresh/atl%i_%s/Nsubs_%s_mag_%i_p_%d_cp_%i_p_%d',atl,AtlNames{atl},metric,...
                    round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp);
                ...
                fprintf('[%s] corr number of pairs, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                ofname = sprintf('figures/T14_allatl_noThresh/atl%i_%s/Npairs_%s_magVar_%i_p_%d_cp_%i_p_%d',...
                ...
                fprintf('[%s] corr number of patients, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                ofname = sprintf('figures/T14_allatl_noThresh/atl%i_%s/Nsubs_%s_magVar_%i_p_%d_cp_%i_p_%d',atl,AtlNames{atl},metric,...
                    round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp);
                ofname = replace(ofname,'.','p');
                print(h,ofname,'-depsc');
                ...
                ofname = sprintf('figures/T14_allatl_noThresh/atl%i_%s/mag_%s_magVar_%i_p_%d',atl,AtlNames{atl},metric,...
                ...
                axis tight;
                %return
                ofname = sprintf('figures/T14_allatl_noThresh/atl%i_%s/dist_%s_mag_%i_p_%d',atl,AtlNames{atl},metric,...
                    round(1000*corr_val_cp),corr_pval_cp);
                ofname = replace(ofname,'.','p');
                print(h,ofname,'-depsc');
                close(h);
...
%                 plot(x_bins,n_triu/trapz(x_bins,n_triu),'black--');
                ...
                box off;
                set(gca,'TickDir','out');
                print(h,sprintf('figures/T14_allatl_noThresh/atl%i_%s/intra_r-%i_inter_r-%i_%s_ranksum-%i_ttest-%i',...
