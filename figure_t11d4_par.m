...
if (~trig_all)
    roi1 = 'parsopercularis';
    %roi1 = 'parstriangularis';
    %roi1 = 'middletemporal';
    %roi1 = 'superiortemporal';
...
%                     col_elec_nosig,'MarkerSize',size_elec_hi)
        ...
        for ii1 = 1:(ecog.n_bchan-1)
            for ii2 = (ii1+1):ecog.n_bchan
                ...
                has_roi_f = ( strcmp(atl_labelsL{ecog.bip(ii1,1)},roi_1)  );
                    
%                 has_roi_f = ( strcmp(atl_labelsL{ecog.bip(ii1,1)},roi_1)  )...
%                         & ( strcmp(atl_labelsL{ecog.bip(ii1,2)},roi_1)  );
...
%                     size_elec_hi_plt = size_elec_hi * (1 + 1*ct(countL));
...
%                     end
                    ...
                    if (trig_covered_rois)
                        covered_rois = unique([covered_rois; elec_xy_sig_roi]);
                    end
...
% 
            ...
            if (strcmp(fig_fmt,'-dpng'))
                print(h,sprintf('figures/T11d4/%s_%s_%s',pname,'sym',magoct),fig_fmt,'-r300');
            else
                print(h,sprintf('figures/T11d4/%s_%s_%s',pname,'sym',magoct),fig_fmt);
            end
            if (trig_eps)
                print(h,sprintf('figures/T11d4/%s_%s_%s',pname,'sym',magoct),'-depsc');
            end
            %return
            close(h);
        end
