...
load('annotation_paths2.mat');
ARTIFACT_CODE_SEIZURE = 6;

art_idx_dir = 'art_stim';
art_idx_dir_out = 'art_nosz';
trig_write = true;
...
for i_buf = 1:length(Buffer)
    ...
    for i = 1:length(outL)
        ...
        for j = 1:length(sid_idx)
            ...
            a_start = idx_a - offset_a;
            a_end = idx_a + offset_a;
            ai_start = idx_ai - offset_ai;
        ...
        %fprintf('\tFraction marked as seizure (n): %i\n',numel(is_sz_a));
        %fprintf('\tFraction marked as seizure (pairwise): %.12f\n',frac_sz_ai);
        %fprintf('\tFraction marked as seizure (pairwise n): %i\n',numel(is_sz_ai));
        fprintf('%.12f,',frac_sz_a);
        fprintf('%i,',numel(is_sz_a));
        fprintf('%.12f,',frac_sz_ai);
        fprintf('%i\n',numel(is_sz_ai));

        if (trig_write)
            system(sprintf('rsync -avP %s %s',infn,outfn));
