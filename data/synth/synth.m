...
% --- TESTS BEFORE RUN ---
%       coregistration folder for subject must be linked in this directory
%       bip_coverage.m
...
h5dir = '../h5eeg';
h5dirArt = '../h5eeg/artifact_removed';
out_dir = '../h5';
...
for fIdx = 1:n_Files
    ...
    if ((isempty(Ecogs)) || (length(Ecogs{1}) == 0))
    ...
    if (~skipSubject)
        ...
        outfilename = sprintf('%s/%s.h5',out_dir,sid);
        ...
        chunk_len = min([chunk_len, sum(h5_n_samples)]);
