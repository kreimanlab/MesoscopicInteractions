...
%load('xsub/xsub2-pcBroadband_atl-2.mat')
m_threshold = 0;
load(sprintf('xsub/xsub2-%s_atl-%i.mat',metric,ATL_I));
%roi_excl = [1,5];
roi_excl = [];
...
A.saveFig(h,sprintf('./figures/adjacency_matrix_atlas/xsub_thresh-%i_%s_%s',...
    m_threshold*1000,metric,at_name));
...
A.saveFig(h2,sprintf('./figures/adjacency_matrix_atlas/xsub_thresh-%i_%s_%s_dist',...
  m_threshold*1000,metric,at_name));
