...
% thresholding
atl_id_str = '2';
tail = 'AdjCTthresh2-57238';
fname_broad = sprintf('xsub/xsub2-pcBroadband_atl-%s_%s.mat',atl_id_str,tail);
fname_delta = sprintf('xsub/xsub2-pcDelta_atl-%s_%s.mat',atl_id_str,tail);
fname_theta = sprintf('xsub/xsub2-pcTheta_atl-%s_%s.mat',atl_id_str,tail);
fname_alpha = sprintf('xsub/xsub2-pcAlpha_atl-%s_%s.mat',atl_id_str,tail);
fname_beta = sprintf('xsub/xsub2-pcBeta_atl-%s_%s.mat',atl_id_str,tail);
fname_gamma = sprintf('xsub/xsub2-pcGamma_atl-%s_%s.mat',atl_id_str,tail);
...
save('xsub_coh_isI','AdjM_isI','m_threshold');
...
ax2 = A.plotAdjElectrodeDistThresh(ax1, 1, AdjM, ...
    sprintf('Functional Interactivity, %s, %s, %s',...
    metric,at_name,tail), inf(size(AdjM)), AdjCS, 0.5, AT_c, atl_ind); %A.const.P_VALUE_CT
...
A.saveFig(h,sprintf('./figures/xsub_coh/xsub_coh_%s_%s_%s',...
    metric,at_name,tail));
...
A.saveFig(h2,sprintf('./figures/xsub_coh/xsub_coh_%s_%s_%s_dist',...
  metric,at_name,tail));

Br.bchan = adjct_bchan;
Br.sub = adjct_sub;
Br.xsubM = adjct_xsubM;
Br.xsub = adjct_xsub;
Br.dist = adjct_dist;
Br.isI = AdjM_isI;
Br.AdjM = AdjM;
...
load(fname_delta);
plot_xsub_cohf_AdjCTthresh;
De.bchan = adjct_bchan;
De.sub = adjct_sub;
De.xsubM = adjct_xsubM;
De.xsub = adjct_xsub;
De.dist = adjct_dist;
De.isI = AdjM_isI;
De.AdjM = AdjM;
...
load(fname_theta);
plot_xsub_cohf_AdjCTthresh;
Th.bchan = adjct_bchan;
Th.sub = adjct_sub;
Th.xsubM = adjct_xsubM;
Th.xsub = adjct_xsub;
Th.dist = adjct_dist;
Th.isI = AdjM_isI;
Th.AdjM = AdjM;
...
load(fname_alpha);
plot_xsub_cohf_AdjCTthresh;
Al.bchan = adjct_bchan;
Al.sub = adjct_sub;
Al.xsubM = adjct_xsubM;
Al.xsub = adjct_xsub;
Al.dist = adjct_dist;
Al.isI = AdjM_isI;
Al.AdjM = AdjM;
...
load(fname_beta);
plot_xsub_cohf_AdjCTthresh;
Be.bchan = adjct_bchan;
Be.sub = adjct_sub;
Be.xsubM = adjct_xsubM;
Be.xsub = adjct_xsub;
Be.dist = adjct_dist;
Be.isI = AdjM_isI;
Be.AdjM = AdjM;
...
load(fname_gamma);
plot_xsub_cohf_AdjCTthresh;
Ga.bchan = adjct_bchan;
Ga.sub = adjct_sub;
Ga.xsubM = adjct_xsubM;
Ga.xsub = adjct_xsub;
Ga.dist = adjct_dist;
Ga.isI = AdjM_isI;
Ga.AdjM = AdjM;
...
close all;
save(sprintf('xsub/xsub_coh_all_%s',tail),'-v7.3')
