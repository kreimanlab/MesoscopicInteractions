...
if __name__ == "__main__":
    ...
    if (len(sys.argv) == 4):
        print('[*] Using patient: '+sys.argv[1])
    else:
    ...
    if (('Leibniz' in host) or ('seldon' in host)):
        #OUT_DIR = '/Volumes/RawData/data/results'
        OUT_DIR = './results'
        #h5fname = '/Volumes/RawData/scripts/synth/out/' + sys.argv[1] + '.h5'
        ...
        if (not Path(h5fname).is_file()):
            ...
            h5fname = '/media/klab/44/h5/' + sys.argv[1] + '.h5'
    ...
    if ((METRIC == 'sc') or (METRIC == 'pc')):
        N_BANDS = 6
        Rf = numpy.zeros((N_GRAPH*N_BANDS,n_comb))
    else:
        Rf = numpy.zeros((N_GRAPH,n_comb))
    ...
    for parIdx in range(0,n_samples,r_rows):
        ...
        if (doPartial):
            ...
            if (USE_CPU):
                ...
                r_coh = coherence_gpu(Xbip,int(round(2*fs)),int(round(fs)),thr)
        else:
            r_cols = int(len(e1))
        ...
        if (TEST):
            if (parCount == 20):
                Rf_t = Rf[0:(N_BANDS*parCount),:]
                print(Rf_t,Rf_t.shape)
                Rp1 = numpy.reshape(Rf_t[:,2],(N_BANDS,parCount),order='F')
                print(Rp1,Rp1.shape)
                Rp2 = numpy.reshape(Rf_t[:,1870],(N_BANDS,parCount),order='F')
                print(Rp2,Rp2.shape)
                plt.subplot(2,1,1)
                plt.plot(numpy.sqrt(Rp1[5,:]),'ko',markersize=1)
                plt.xlabel('Minutes')
                plt.ylabel('Broadband Coherence')
                plt.xlim([0,(parCount-1)])
                plt.ylim([-0.8,0.8])
                plt.subplot(2,1,2)
                plt.plot(numpy.sqrt(Rp2[5,:]),'ko',markersize=1)
                plt.xlabel('Minutes')
                plt.ylabel('Broadband Coherence')
                plt.xlim([0,(parCount-1)])
                plt.ylim([-0.8,0.8])
    ...
    if (doCoherence):
        ...
        for i in range(N_BANDS):
            ...
            Rcoh = numpy.zeros((N_GRAPH,n_comb))
