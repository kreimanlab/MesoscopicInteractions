...
def coherence_gpu(v, nperseg, fs, thr):
    #
    ...
    # Segmentation
    noverlap = int(nperseg * 0.5) # rounded
    ...
    # res_dev is a pyopencl.array.Array object
    Vf = res_dev.get()
...
def coherence2_gpu(v, v2, nperseg, fs, thr):
    #
    #   GPU implementation of magnitude square coherence estimate using Welch's
    #   method based on scipy.signal.coherence. Reports the pair-wise coherence
    #   of a multivariate input averaged over defined frequency bands.
...
def coherence_gpu_test(v, nperseg, fs, thr):
    ...
    # Compare to scipy
    f_sp, Vp_sp = scipy.signal.welch(v, fs=fs, window='hanning', nperseg=nperseg, detrend=False, axis=0)
    print('\nVp_sp pxx:')
...
def coherence(v, fs):
    ...
    for i in range(1, n_chan):
        for j in range(i+1, n_chan+1):
            f, C2 = signal.coherence(v[:,i-1],v[:,j-1],fs=fs, nperseg=fs*2, detrend=False, window='hamming')
            C2 = numpy.sqrt(C2)
...
def coherence2(v, w, fs):
    ...
    for i in range(1, n_chan):
        for j in range(i+1, n_chan+1):
            f, C2 = signal.coherence(v[:,i-1],w[:,j-1],fs=fs, nperseg=fs*2, detrend=False, window='hamming')
            C2 = numpy.sqrt(C2)
            mask_del = ((f > DEL_S) & (f < DEL_E))
            mask_the = ((f >= THE_S) & (f < THE_E))
            mask_alp = ((f >= ALP_S) & (f < ALP_E))
...
def main():
    ...
    w = 60
    r_idx = random.randint(0,(n_samples-round(w*fs)))
    #r_idx = 0
    r_rows = int(round(w*fs))
    ...
    t0 = time.time()
    r_coha,ph = coherence_gpu(Xbip, 2*fs, fs, thr)
    t_gpu = time.time() - t0
    print('gpu coherence: \n',r_coha[0,0:10],'...',r_coha[0,-1])
    print('gpu time: ',t_gpu)

    if (False):
        t0 = time.time()
        r_coh = coherence(Xbip, fs)
        t_cpu = time.time() - t0
        print('scipy coherence: \n',r_coh[0,0:10],'...',r_coh[0,-1])
        print('cpu time: ',t_cpu)
        print('gpu speedup: ',t_cpu/t_gpu)

        #print('GPU\n',numpy.sqrt(r_coha))
        #print('CPU\n',numpy.sqrt(r_coh))
        d = numpy.abs(numpy.sqrt(r_coha) - numpy.sqrt(r_coh)).ravel()
        #print(d,d.shape)
        d = sorted(d)
    ...
    t0 = time.time()
    r_coh2a = coherence2_gpu(Xbip, Ybip, 2*fs, fs, thr)
    t_gpu = time.time() - t0
    print('gpu time: ',t_gpu)
    ...
    if (False):
        t0 = time.time()
        r_coh2 = coherence2(Xbip, Ybip, fs)
        t_cpu = time.time() - t0
        print('cpu time: ',t_cpu)
        print('gpu speedup: ',t_cpu/t_gpu)

        d = numpy.abs(numpy.sqrt(r_coh2a) - numpy.sqrt(r_coh2)).ravel()
        d = sorted(d)
        #d_mu = numpy.mean(numpy.abs(r_coha - r_coh))
        #d_sig = numpy.std(numpy.abs(r_coha - r_coh))
