...
%           'mk' - Markov-Kennedy 2014 Macaque Atlas
%           'fve' - Felleman-Van Essen 1991 Macaque Atlas
%           'fvea' - Felleman-Van Essen all Macaque Atlas
...
SMOOTH_FACE = false; % Use geometric smoothing when rendering surface
BLUR_ROI = false; % Blur face colors between ROI boundaries
ATLAS_SATURATION = 1; % Multiplier between 0,1 for atlas color saturation
ATLAS_TINT = [1 1 1]; % Tint color
...
for hIdx = 1:length(Hemi)
    ...
    p.AmbientStrength = 0.3 + 0.05*(length(Hemi)-1);
    p.DiffuseStrength = 0.4 - 0.25*(length(Hemi)-1);
