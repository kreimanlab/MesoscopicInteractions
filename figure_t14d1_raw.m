...
for iM = 1 %1:length(metrics)
    Caz = load(sprintf('%s/xsub_out_all_%i_atl2.mat',dir_cacheL,iM));
    AdjAtl = Caz.AdjAtl;
    AdjAtl_sid = Caz.AdjAtl_sid;
        
    for iSub = 3 %1:length(SubjectsL)
        ...
        load(sprintf('%s/xsub_out_%s_%i_atl2.mat',dir_cacheL,sid,iM));
        sid = sid_sav;
        iSub = iSub_sav;
...
%         end
        ...
        ofi = fopen(sprintf('figures/T14d1/Figure_W2-%iA_Adj_%s_%s_%s_sig-%i-of-%i.txt',...
        ...
        sid = 'mX';
        print(h,sprintf('figures/T14d1/Figure_W2-%iA_Adj_%s_%s_%s_sig-%i-of-%i',...
            sid_int,metric,sid,ss,n_sig,(n_sig + n_nosig)),fig_fmt,'-r300');
        if (trig_eps)
            %print(h,sprintf('figures/T14d1/Figure_W2-%iA_Adj_%s_%s_%s',sid_int,metric,sid,ss),'-depsc');
            print(h,sprintf('figures/T14d1/Figure_W2-%iA_Adj_%s_%s_%s_sig-%i-of-%i',...
                sid_int,metric,sid,ss,n_sig,(n_sig + n_nosig)),fig_fmt,'-r300');
        end
