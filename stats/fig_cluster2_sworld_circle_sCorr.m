...
for iM = 1%[1:5] %[1 5]
    ...
    fn_ca = sprintf('./cache/xsub_out_all_%i_atl2_sCorr.mat',iM);
    Ca = load(fn_ca);
    ...
    fn_ca4 = sprintf('./cache/figure_t14_%i_atl2_Desikan-Killiany_sCorr',iM);
    Ca4 = load(fn_ca4);
...
%     rois2 = rois2(cov_idx);
    ...
    if (trig_perform_random_init)
        ...
        save('fig_cluster2_sworld_circle_E_sCorr','E');
    else
        % Load existing randomly initialized network
        Eca = load('fig_cluster2_sworld_circle_E_sCorr');
        E = Eca.E;
    end
    ...
    % save
    print(h,sprintf('./figures/fig_cluster2_sworld_circle_metric-%i_sCorr',iM),'-depsc');
    print(h,sprintf('./figures/fig_cluster2_sworld_circle_metric-%i_sCorr',iM),'-dpng','-r900');
    close(h);
...
%             fprintf('\tSWI = %.6f\n',I);
        ...
        for i = 1:length(Ca.Subjects)
            sid = Ca.Subjects{i};
            Cas = load(sprintf('./cache/xsub_out_%s_%i_sCorr.mat',sid,iM));
        end
...
%         fprintf('\tSWI = %.6f\n',I);
    ...
    save(sprintf('cache/fig_cluster2_sworld_circle-%i_sCorr.mat',iM));
...
%     
    ...
    % SWP raw
    if (iM == 1)
        save('./cache/Adj_plt_mSu_red_sCorr','Adj_plt_mSu_red','Adj_plt_mSu_red_rois');
    end
    %return
    % --------------------------------------------------------------------
    ...
    for iq = 1:length(A_queue)
        ...
        Thrs = Thrs(2:(end-1));
        fn_cache_dk = sprintf('fig_cluster2_sworld_circle_swp_dk_%i_macaque-%i_sCorr',iM,iq);
    ...
    Thrs = linspace(0.15,max_density,n_pts+2);
    Thrs = Thrs(2:(end-1));
    fn_cache_dk = sprintf('fig_cluster2_sworld_circle_swp_dk_%i_sCorr',iM);
    if (~exist([fn_cache_dk,'.mat'],'file'))
        for thr = Thrs
            nsamps = 10000;
            SWP = zeros(1,nsamps);
...
%     xtickangle(90);
    ...
    print(h,sprintf('./figures/fig_cluster2_sworld_circle_im-%i_swp_sCorr',iM),'-depsc','-r900');
    print(h,sprintf('./figures/fig_cluster2_sworld_circle_im-%i_swp_sCorr',iM),'-dpng','-r900');
    close(h);
    %return
%     
%     % Brain metrics
...
% 	Lr	72.035557, std 3.16e+00, n=10000, 95%CI: 6.637e+01 - 7.883e+01
% 	Ll	280.227031, std 4.22e+00, n=10000, 95%CI: 2.719e+02 - 2.882e+02
% 	C	0.002804, std 4.58e-16, n=10000, 95%CI: 2.804e-03 - 2.804e-03
% 	Cr	0.000755, std 2.80e-05, n=10000, 95%CI: 7.020e-04 - 8.118e-04
% 	Cl	0.002825, std 5.11e-06, n=10000, 95%CI: 2.816e-03 - 2.836e-03
% 	DC	0.010196, std 2.45e-03, n=10000, 95%CI: 5.887e-03 - 1.554e-02
% 	DL	0.951098, std 1.91e-02, n=10000, 95%CI: 9.153e-01 - 9.904e-01
% 	SWP	0.327431, std 1.35e-02, n=10000, 95%CI: 2.996e-01 - 3.527e-01
% 	SWI	0.048331, std 1.91e-02, n=10000, 95%CI: 9.429e-03 - 8.376e-02
% 	delta	0.986341, std 3.31e-03, n=10000, 95%CI: 9.792e-01 - 9.921e-01
% 	gamma	3.720221, std 1.38e-01, n=10000, 95%CI: 3.454e+00 - 3.994e+00
% 	lambda	3.754962, std 1.62e-01, n=10000, 95%CI: 3.424e+00 - 4.067e+00
% 	sigma	0.992408, std 5.31e-02, n=10000, 95%CI: 8.939e-01 - 1.101e+00
% 	omega	-0.725711, std 1.19e-02, n=10000, 95%CI: -7.468e-01 - -7.002e-01
% [*] Markov-Kennedy SWP (n=91), weighted
% 	n_samp: 100
% 	SWP: 0.327379 (+- 0.013187)(95% CI: 0.304681 - 0.347219, n=100)
% 	deltaC: 0.010205 (+- 0.002255)
% 	deltaL: 0.951172 (+- 0.018652)
% 	deviance: 0.986330 (+- 0.003053)(95% CI: 0.980788 - 0.991281, n=100)
...
% 	Lr	8.497573, std 2.94e-02, n=10000, 95%CI: 8.442e+00 - 8.556e+00
% 	Ll	8.848123, std 6.94e-03, n=10000, 95%CI: 8.834e+00 - 8.862e+00
% 	C	0.567225, std 4.83e-14, n=10000, 95%CI: 5.672e-01 - 5.672e-01
% 	Cr	0.477152, std 1.24e-02, n=10000, 95%CI: 4.553e-01 - 5.041e-01
% 	Cl	0.492267, std 5.54e-05, n=10000, 95%CI: 4.922e-01 - 4.924e-01
% 	DC	0.000000, std 00, n=10000, 95%CI: 000 - 000
% 	DL	0.907598, std 1.98e-02, n=10000, 95%CI: 8.675e-01 - 9.458e-01
% 	SWP	0.358231, std 1.40e-02, n=10000, 95%CI: 3.312e-01 - 3.865e-01
% 	SWI	0.647035, std 1.36e+01, n=10000, 95%CI: -3.504e+00 - 4.184e+00
% 	delta	1.000000, std 00, n=10000, 95%CI: 001 - 001
% 	gamma	1.189564, std 3.06e-02, n=10000, 95%CI: 1.125e+00 - 1.246e+00
% 	lambda	1.037465, std 3.58e-03, n=10000, 95%CI: 1.030e+00 - 1.044e+00
% 	sigma	1.146590, std 2.85e-02, n=10000, 95%CI: 1.086e+00 - 1.199e+00
% 	omega	-0.188371, std 3.33e-03, n=10000, 95%CI: -1.947e-01 - -1.817e-01
% [*] Found cache: fig_cluster2_sworld_circle_swp_dk_1_macaque-1_sCorr, loading..
% [*] Calculating human SWP with 10000 permutations..
% [*] Adj, nodes=20
% 	L	3.722281, std 1.47e-13, n=10000, 95%CI: 3.722e+00 - 3.722e+00
% 	Lr	3.470384, std 2.86e-02, n=10000, 95%CI: 3.427e+00 - 3.538e+00
% 	Ll	3.375861, std 2.06e-03, n=10000, 95%CI: 3.372e+00 - 3.380e+00
% 	C	0.685342, std 1.42e-13, n=10000, 95%CI: 6.853e-01 - 6.853e-01
% 	Cr	0.328137, std 2.92e-02, n=10000, 95%CI: 2.740e-01 - 3.893e-01
% 	Cl	0.540083, std 9.22e-05, n=10000, 95%CI: 5.399e-01 - 5.403e-01
% 	DC	0.000000, std 00, n=10000, 95%CI: 000 - 000
% 	DL	-2.982456, std 1.21e+00, n=10000, 95%CI: -5.804e+00 - -1.131e+00
% 	SWP	-1.110471, std 8.49e-01, n=10000, 95%CI: -3.104e+00 - 2.000e-01
% 	SWI	6.688374, std 2.34e+00, n=10000, 95%CI: 3.802e+00 - 1.118e+01
% 	delta	-2.995400, std 1.34e-01, n=10000, 95%CI: -003 - -003
% 	gamma	2.105135, std 1.87e-01, n=10000, 95%CI: 1.760e+00 - 2.502e+00
% 	lambda	1.072657, std 8.77e-03, n=10000, 95%CI: 1.052e+00 - 1.086e+00
% 	sigma	1.962081, std 1.69e-01, n=10000, 95%CI: 1.653e+00 - 2.320e+00
% 	omega	-0.336629, std 7.68e-03, n=10000, 95%CI: -3.483e-01 - -3.186e-01
% [*] Found cache: fig_cluster2_sworld_circle_swp_dk_1_sCorr, loading..
% Overall SWP: 0.617640 (+- 1.445171)
% Overall SWP: -0.126084 (+- 0.370132)
