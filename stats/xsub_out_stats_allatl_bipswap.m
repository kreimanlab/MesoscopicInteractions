...
%         return
        ...
        %==================================================================
        bip_tmp = ecog.bip;
        bip_tmp(:,1) = ecog.bip(:,2);
        bip_tmp(:,2) = ecog.bip(:,1);
        bip_tmp(:,4:6) = ecog.bip(:,7:9);
        bip_tmp(:,7:9) = ecog.bip(:,4:6);
...
%                     b2c2(k) = ecog.bip(chan2(k)+1,2);
                    b1c1(k) = bip_tmp(chan1(k)+1,1);
                    b1c2(k) = bip_tmp(chan1(k)+1,2);
                    b2c1(k) = bip_tmp(chan2(k)+1,1);
                    b2c2(k) = bip_tmp(chan2(k)+1,2);
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_atl%i_bipswap',sid,metrici,atl),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_atl%i_bipswap',metrici,atl),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
function save_tmp(sid, R_null, atl, metrici)
    save(sprintf('cache/%s_R_null_atl%i_%i_bipswap',sid,atl,metrici),'R_null','-v6');
end
