...
for ist = 1:length(SurfT)
    ...
    %     end
        ...
        if (trig_plot_mag_dist)
            ...
            print(h2,sprintf('figures/cluster/mag_dist_nsub-%i',n_sidintu),'-depsc');
            close(h2);
        end
...
%             ax_clone6 = copyobj(hax1, hfig);
           ...
           % return
            print(hfig,sprintf('figures/cluster/%s_nbip-%i_nedge-%i',surf_type,length(E),length(D)),'-dpng','-r300');
            close(hfig);
        end
