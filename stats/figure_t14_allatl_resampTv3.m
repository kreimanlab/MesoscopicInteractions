...
system('mkdir figures/T14_allatl_resampTv3');
...
for atl = [2] %1:20
    ...
    system(sprintf('mkdir figures/T14_allatl_resampTv3/atl%i_%s',atl,AtlNames{atl}));
    ...
    for iM = [5] %1:length(metrics) % [1 5] %
        ...
        % for permutation
        n_perm_max = 130;
        Stats = nan(8,n_perm_max);
        for iPerm = 1:n_perm_max
            ...
            for repL = [1 2]
                % Load human cache
                %Ca_hum = load(sprintf('%s/xsub_out_all_%i_atl%i_resampTv3-%i-1.mat',dir_cacheL,iM,atl,repL));
                Ca_hum = load(sprintf('%s/xsub_out_all_%i_atl%i_resampTv3-%i-%i.mat',dir_cacheL,iM,atl,repL,iPerm));
        ...
        %adjct_dist_cl = adjct_dist(ind_hum2mac,ind_hum2mac);
                    ...
                    %                 yc = yc + 1;
                            ...
                            save(sprintf('./cache/figure_t14_%i_atl%i_%s_resampTv3_rep%i',iM,atl,AtlNames{atl},repL),'AdjMag','AdjMagVar','Adj_plt','Adj_plt2','Adj_plt2_cl','Y','Z','M','cluster_i','rois_plt','atl');
        ...
        %                 end
            ...
            %             xtickangle(90);
                ...
                %         colormap(map);
                %         if (((~iStatssnan(minv)) && (~isnan(maxv))) && (minv ~= maxv))
                ...
                %         end
                        ...
                        %
                        print(h,sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Adj_%s_%s_rep%i',atl,AtlNames{atl},metric,ss,repL),fig_fmt);
                        if (trig_eps)
                            print(h,sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Adj_%s_%s_rep%i',atl,AtlNames{atl},metric,ss,repL),'-depsc');
                        end
    ...
    %                     end
        ...
        %                     end
        %                     axis off;
        %                     print(h,sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Adj_%s_%s_dendro',atl,AtlNames{atl},metric,ss),fig_fmt);
        %                     if (trig_eps)
        %                         print(h,sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Adj_%s_%s_dendro',atl,AtlNames{atl},metric,ss),'-depsc');
        %                     end
        %                     close(h);
        %                 end
                        ...
                        if (iii2 == 1)
                            ...
                            fprintf('[*] saved file: %s\n',sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss));
        ...
        %                 fprintf('[*] Coherence Stdev n: %.2f\n',numel(V_std));
            ...
            %             axis tight;
                        ...
                        ofname = sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Npairs_%s_mag_%i_p_%d_cp_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
            ...
            %             axis tight;
                        ...
                        ofname = sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Nsubs_%s_mag_%i_p_%d_cp_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                            round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp,repL);
                        ...
                        fprintf('[%s] corr number of pairs, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                        ofname = sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Npairs_%s_magVar_%i_p_%d_cp_%i_p_%d_rep%i',...
                        ...
                        fprintf('[%s] corr number of patients, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                        ofname = sprintf('figures/T14_allatl_resampTv3/atl%i_%s/Nsubs_%s_magVar_%i_p_%d_cp_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                            round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp,repL);
                        ofname = replace(ofname,'.','p');
                        print(h,ofname,'-depsc');
                        ...
                        ofname = sprintf('figures/T14_allatl_resampTv3/atl%i_%s/mag_%s_magVar_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                        ...
                        axis tight;
                        %return
                        ofname = sprintf('figures/T14_allatl_resampTv3/atl%i_%s/dist_%s_mag_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                            round(1000*corr_val_cp),corr_pval_cp,repL);
                        ofname = replace(ofname,'.','p');
                        print(h,ofname,'-depsc');
                        close(h);
                        ...
                        box off;
                        set(gca,'TickDir','out');
                        print(h,sprintf('figures/T14_allatl_resampTv3/atl%i_%s/intra_inter_%s_ranksum-%i_ttest-%i_rep%i',atl,AtlNames{atl},metric,...
            ...
            AdjAA_1 = BpairsRep{1}; % rep low
            AdjAA_2 = BpairsRep{2}; % rep hi
            vAA_1 = [];
            vAA_2 = [];
    ...
    %         vAA_2 = vAA_2(idx_both);
            ...
            AdjMagRep1 = AdjMagRep{1}; % rep lo
            AdjMagRep1 = AdjMagRep1(known_idx,known_idx);
            AdjMagRep2 = AdjMagRep{2}; % rep hi
    ...
    %         fprintf('[*] stdev diff: %0.2f%%\n',std(100*f_d));
            ...
            [p1,~,~] = ranksum(v_A1(ind_both),v_A2(ind_both));
            fprintf('\tRanksum\tp=%.4d\n',p1);
            [~,p2,~] = ttest(v_A1(ind_both),v_A2(ind_both),'tail','both');
            fprintf('\tT-test\tp=%.4d\n',p2);
            fprintf('\tn=%i\n',length(v_A1))
            n_both = sum(ind_both);
            fprintf('\tboth n=%i\n',n_both);
            ...
            % Store
            Stats(1,iPerm) = mean(abs(v_d3(ind_both)));
            Stats(2,iPerm) = std(abs(v_d3(ind_both)));
            Stats(3,iPerm) = nanmean(100*abs(f_d(ind_both)));
            Stats(4,iPerm) = nanstd(100*abs(f_d(ind_both)));
            Stats(5,iPerm) = length(f_d(ind_both));
            Stats(6,iPerm) = p1;
            Stats(7,iPerm) = p2;
            Stats(8,iPerm) = length(v_A1);
    ...
    %         fprintf('[*] stdev diff: %0.6f\n',std(v_d2));
        ...
        fprintf('======================================================\n');
        fprintf('Num permutations: %i\n',n_perm_max);
        fprintf('Mean diff: %.6f +- %.6f\n',median(Stats(1,:)),std(Stats(1,:)));
        fprintf('Stdev diff: %.6f +- %.6f\n',median(Stats(2,:)),std(Stats(2,:)));
        fprintf('Mean diff: %.2f%% +- %.2f%%\n',median(Stats(3,:)),std(Stats(3,:)));
        fprintf('Stdev diff: %.2f%% +- %.2f%%\n',median(Stats(4,:)),std(Stats(4,:)));
        fprintf('Both n: %.4f +- %.4f\n',median(Stats(5,:)),std(Stats(5,:)));
        fprintf('Ranksum p: %.4d +- %.4d\n',median(Stats(6,:)),std(Stats(6,:)));
        fprintf('T-test p: %.4d +- %.4d\n',median(Stats(7,:)),std(Stats(7,:)));
        fprintf('Total n: %.4f +- %.4f\n',median(Stats(8,:)),std(Stats(8,:)));
    end
end
...
% Broadband
% ======================================================
% Num permutations: 130
% Mean diff: 0.046583 +- 0.011018
% Stdev diff: 0.044723 +- 0.010872
% Mean diff: 17.24% +- 4.72%
% Stdev diff: 14.16% +- 6.48%
% Both n: 140.5000 +- 16.8720
% Ranksum p: 5.3123e-04 +- 2.1098e-01
% T-test p: 2.0312e-09 +- 1.6768e-01
% Total n: 193.0000 +- 0.0000

% Gamma
% ======================================================
% Num permutations: 130
% Mean diff: 0.054811 +- 0.010385
% Stdev diff: 0.046831 +- 0.010877
% Mean diff: 19.45% +- 3.87%
% Stdev diff: 13.30% +- 4.96%
% Both n: 120.0000 +- 17.5761
% Ranksum p: 7.6110e-06 +- 1.8169e-01
% T-test p: 6.0832e-12 +- 1.9710e-01
% Total n: 184.0000 +- 0.0000
...
% Remove first half of data. ( AdjMagFull - AdjMagRep1;)
% [*] mean diff: 0.156021
% [*] stdev diff: 0.160574
% [*] mean diff: 48.11%
% [*] stdev diff: 46.84%
% Compare first half with second. (AdjMagRep1 - AdjMagRep2;)
% [*] mean diff: -0.043265
% [*] stdev diff: 0.068159
% [*] mean diff: 13.67%
% [*] stdev diff: 25.79%
