...
for iM = 1 %[1 5]
    ...
    % define short ROI names
    def_rois_short;
    
    % load figure t14 regions
    CaDK = load(sprintf('./cache/figure_t14_%i_atl%i_%s',iM,2,'Desikan-Killiany'));
    roi_list_formal = CaDK.rois_plt(CaDK.cluster_i);
    dk_formal = cell(size(ct.struct_names));
    for idf = 1:length(dk_formal)
        dk_formal{idf} = convertRoiDK(ct.struct_names{idf});
    end
    roi_list_short = cell(size(roi_list_formal));
    for ilf = 1:length(roi_list_formal)
        roi_list_short{ilf} = ct.struct_names{(strcmp(dk_formal,roi_list_formal{ilf}))};
    end
    
    % input regions to color
    %roi_list = {b1_roi,b2_roi};
    roi_list = roi_list_short;
    
    
    CaSW = load(sprintf('cache/fig_cluster2_sworld_circle-%i.mat',iM));
    roi_list = CaSW.rois2_raw;
    
    
    vtx_r1 = false(length(l),1);
    vtx_r2 = false(length(l),1);
    %vtx_rx = false(length(l),length(roi_list));
    for ihi = 1:ct.numEntries
        roi_id = ct.table(ihi,end);
        
        cond_rx = nan(1,length(roi_list));
        for ihi2 = 1:length(cond_rx)
            cond_rx(ihi2) = strcmp(ct.struct_names{ihi},roi_list{ihi2});
        end
    ...
    col_rx = nan(length(roi_list),3);
    for icr = 1:length(roi_list)
        col_rx(icr,:) = ct.table(strcmp(ct.struct_names,roi_list{icr}),1:3) / 255;
    end
...
%     
    ...
    legendColAll = nan(length(roi_list),3);
    for icr = 1:length(roi_list)
        legendColAll(icr,1:3) = col_rx(icr,:);
    end
    %legendStr = {'Superior Temporal','Pars Opercularis'};
    legendStr = cell(size(roi_list));
    for icr = 1:length(legendStr)
        legendStr{icr} = convertRoiDK(roi_list{icr});
    end
    
    legendStr = CaSW.rois2_ab;
    for il = 1:length(roi_list)
        box_width = 0.010;
        box_vert_spacer = 6 * box_width * box_width;
        horiz_offset = -0.05;
        horiz_text_offset = -1 * (box_width) + 0.006;
        horiz_pos = pos{1}(1) + pos{1}(3) + horiz_offset;
        vert_offset =  - il * (box_width + box_vert_spacer);
        vert_pos = pos{1}(2) + pos{1}(4) + vert_offset;
        vert_text_offset = 0.009;
        annotation('rectangle',[horiz_pos + horiz_text_offset, vert_pos, box_width, 0.9*box_width*(paper_width/paper_height)],...
            'FaceColor',legendColAll(il,:),'EdgeColor','black','LineWidth',1);
        annotation('textbox',[horiz_pos,vert_pos-vert_text_offset, 18*box_width, 2*box_width*(paper_width/paper_height)],...
            'String',legendStr{il},'EdgeColor','none',...
            'FontSize',4,'FontName','Helvetica','VerticalAlignment','middle')
    end
    ...
    mkdir('./figures/T22');
    print(h,sprintf('figures/T22/Overlap_%i',iM),'-depsc','-r1800');
    print(h,sprintf('figures/T22/Overlap_%i',iM),'-dpng','-r1800');
    close(h);
...
%     
%     
%     print(h,sprintf('figures/T22/Overlap_%i_adj',iM),'-depsc','-r400');
%     print(h,sprintf('figures/T22/Overlap_%i_adj',iM),'-dpng','-r400');
%     close(h);
%     
%     save(sprintf('./cache/figure_t22_%i',iM));
