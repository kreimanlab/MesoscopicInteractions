...
load('./markov_data/AdjMKnew.mat');
AdjMKflne = D_anat_new.AdjMKflne;
AdjMKneurons = D_anat_new.AdjMKneurons;
AdjMKflne(isinf(AdjMKflne)) = nan;
AdjMKneurons(isinf(AdjMKneurons)) = nan;

% calculate flne from neurons
AdjMKneurons_n = nansum(AdjMKneurons,1);
AdjMKflne_redo = nan(size(AdjMKneurons));
for i = 1:length(AdjMKneurons)
    AdjMKflne_redo(i,:) = AdjMKneurons(i,:)./AdjMKneurons_n;
end
AdjMKflne = AdjMKflne_redo;

AdjMKflne(isnan(AdjMKflne)) = 0;
AdjMKneurons(isnan(AdjMKneurons)) = 0;
MK_labels = D_anat_new.MK_labels;
...
print(h,sprintf('figures/T8d3/Corr_mSu_MK_bin_%s_r-%i',metric,round(1000*r_bin)),fig_fmt);
if (trig_eps)
    print(h,sprintf('figures/T8d3/Corr_mSu_MK_bin_%s_r-%i',metric,round(1000*r_bin)),'-depsc');
end
close(h);
...
print(hh,sprintf('figures/T8d3/Corr_mSu_MK_f_%s_r-%i_hist',metric,round(1000*rSrf)),'-dsvg');
...
print(h,sprintf('figures/T8d3/Corr_mSu_MK_f_%s_r-%i',metric,round(1000*rSrf)),fig_fmt);
if (trig_eps)
    print(h,sprintf('figures/T8d3/Corr_mSu_MK_f_%s_r-%i',metric,round(1000*rSrf)),'-depsc');
end
close(h);
...
axis tight;
axis off;
print(h,sprintf('figures/T8d3/Corr_mSu_MK_f_%s_r-%i_dendro',metric,round(1000*rSrf)),'-depsc');
print(h,sprintf('figures/T8d3/Corr_mSu_MK_f_%s_r-%i_dendro',metric,round(1000*rSrf)),'-dpng');
close(h);
...
% Save roi names
ofn = sprintf(sprintf('figures/T8d3/Corr_mSu_MK_f_%s_r-%i.txt',metric,round(1000*rSrf)));
of = fopen(ofn,'w');
