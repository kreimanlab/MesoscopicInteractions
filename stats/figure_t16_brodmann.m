...
mkdir('figures');
mkdir(sprintf('figures/T16_brodmann'));
...
for imm = 1:length(imetrics)
    ...
    for trig_show_path = [0] %[0 1]
        ...
        CaA = load(sprintf('%s/xsub_out_all_%i_atl%i',dir_cache,iM,atl_const));
        ...
        Ca = load(sprintf('%s/xsub_out_%s_%i_atl%i',dir_cache,sid,iM,atl_const));

        % Find ROIs bipolar electrodes map onto
        rois = Ca.C.AtlLabels{atl_const};
        b1c1 = Ca.ecog.bip(b1,:);
        b2c1 = Ca.ecog.bip(b2,:);
        b1_roi = rois{b1c1};
        b2_roi = rois{b2c1};
...
%         end
        ...
        const_alpha_surface = 0.8;
        const_elec_surface = 0*[1 1 1];
        const_elec_low = 0.45*[1 1 1]; % 0.75
...
%         p2.SpecularExponent = 1;
        ...
        for i = (1+1):(n_usub+1)
            ...
            % Highlight areas
            if (trig_color_rois)
                atl_annot_txt = 'PALS_B12_Brodmann';
                [v_a, l, ct] = read_annotation(sprintf('%s/%s/label/%sh.%s.annot',subjects_dir,sid,hemi,atl_annot_txt));
                [n_vert,~] = size(s_vert);
...
%                     'EdgeColor','none','FaceColor',const_col_surface,'FaceAlpha',const_alpha_surface);
    ...
    %     axis off;
        ...
        % Save figure
        print(h,sprintf('figures/T16_brodmann/figure_t16_%i_curve-%i_atl%i_%i',iM,trig_show_path,atl_const,trig_show_subject_num),'-depsc','-r400');
        print(h,sprintf('figures/T16_brodmann/figure_t16_%i_curve-%i_atl%i_%i',iM,trig_show_path,atl_const,trig_show_subject_num),'-dpng','-r400');
...
%     mag_min = min(mag_all);
    ...
    fn_fig = sprintf('figures/T16_brodmann/figure_t16_metric-%i_DK_%i',iM,trig_show_subject_num);
