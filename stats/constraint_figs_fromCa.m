...
fprintf('[*] Loading precomputed constraint cache..\n')
Cac = load('cache/constraint');
fprintf('\tDone.\n')
...
%     end
    ...
    % plot
    close all;
    h = figure('Position',[0,0,300,260],'visible','off');
    ...
    R_coh_sim = [R_coh_sim; r];
    P_coh_sim = [R_coh_sim; p];
    print(h,sprintf('figures/constraint/%i_coh_sim_r_%i_p_%.0d_n-%i',iSub,round(r*1000),p,length(X)),'-dpng','-r900');
    print(h,sprintf('figures/constraint/%i_coh_sim_r_%i_p_%.0d_n-%i',iSub,round(r*1000),p,length(X)),'-depsc');
    
    if (iSub == 3)
        % Network Saturation
        h2 = figure('Position',[0,0,600,260],'visible','off');
        i_comb = 1;
        idx_comb = true(1,n_comb);
        idx_comb(i_comb) = false;
        usage = sum(R_bin(idx_comb,:))/n_comb;
        ...
        box off;
        axis tight;
        print(h2,sprintf('figures/constraint/%i_usage',iSub),'-dpng','-r900');


        % Bipolar pair significance
        h3 = figure('Position',[0,0,600,160],'visible','off');
        i_comb = 3;
        idx_comb = true(1,n_comb);
        idx_comb(i_comb) = false;
        usage = sum(R_bin(idx_comb,:))/n_comb;
        [rho,pval] = corr(usage',R_bin(i_comb,:)','Type','Spearman');
        fprintf('[!] corr usage, r_bin: %.3f, p=%.3d, n=%i\n',rho,pval,length(usage))
        ...
        box off;
        axis tight;
        print(h3,sprintf('figures/constraint/%i_rbin',iSub),'-dpng','-r900');
        ...
        idx_comb(i_comb) = false;
        usage = sum(R_bin(idx_comb,:))/n_comb;
        [rho,pval] = corr(usage',R(i_comb,:)','Type','Spearman');

        fprintf('[!] corr usage, r: %.3f, p=%.3d, n=%i\n',rho,pval,length(usage))
        ...
        box off;
        axis tight;
        print(h3,sprintf('figures/constraint/%i_r',iSub),'-dpng','-r900');
        %return
    end
end
