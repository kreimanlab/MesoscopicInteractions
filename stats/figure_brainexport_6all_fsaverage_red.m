...
for metrici = 1:length(metrics)
    ...
    CaR = load(sprintf('%s/fig_cluster2_reduce_%i_new.mat','cache',metrici));

    rem = [];
    for i = 1:length(CaR.Es)
        lin = CaR.E(i,:);
        if (false)%((abs(lin(3) - 1.161) < 0.001) || (abs(lin(3) - 0.5528) <= 0.001))
            fprintf('[*] Skipped location %i: %.3f, %.3f, %.3f\n',i,lin(3),lin(4),lin(5));
            rem = [rem; i];
        else
            plot3(lin(3),lin(4),lin(5),'black.'); hold on
        end
    end
    E = CaR.E;
    E(rem,:) = [];
    ...
    Paths = {};
    Paths_ind = [];
    Counts = 1:(nchoosek(n_E,2));
    CountsI = zeros(nchoosek(n_E,2),2);
    c = 1;
    for ii = 1:(n_E-1)
        for jj = (ii+1):n_E
            CountsI(c,1) = ii;
    ...
    % 
    %     path = [pt1];
    %     alphas = single(linspace(0,1,max_iter+2));
    ...
    % 
    %     [X,Y,Z] = tubeplot(path(:,1),path(:,2),path(:,3),ones(length(path(:,1))),0.1,3,[0 1 0]);
    %     mesh(X,Y,Z); hold on;
    %     return
%         plot3(path(:,1),path(:,2),path(:,3),'red-');
%         plot3(path(:,1),path(:,2),path(:,3),'magenta.','MarkerSize',7);
%         return
    %     if (c_i > 120)
    %         break
    %     end
        ...
        t = toc;
        fprintf('\t* %i of %i, Est. Total: %.2f hrs\n',count,nchoosek(n_E,2),nchoosek(n_E,2)*t/3600)
    end
    %return

    
    save_name = sprintf('brainexport/red_6all_fsaverage_%i',metrici);
    fprintf('[*] Saving to: %s\n',save_name);
