...
system('mkdir figures/T14_allatl_bipswap');
...
for atl = 2 %1:20 %1:20
    ...
    system(sprintf('mkdir figures/T14_allatl_bipswap/atl%i_%s',atl,AtlNames{atl}));
    
    for iM = 1 %[1 5] %1:length(metrics) % [1 5] %
        metric = metrics{iM};

        % Load human cache
        Ca_hum = load(sprintf('%s/xsub_out_all_%i_atl%i_bipswap.mat',dir_cacheL,iM,atl));
...
%adjct_dist_cl = adjct_dist(ind_hum2mac,ind_hum2mac);
            ...
            %                 yc = yc + 1;
                    ...
                    save(sprintf('./cache/figure_t14_%i_atl%i_%s_bipswap',iM,atl,AtlNames{atl}),'AdjMag','AdjMagVar','Adj_plt','Adj_plt2','Adj_plt_null','Adj_plt2_cl','Y','Z','M','cluster_i','rois_plt','atl');
                    
                    CaDefault = load(sprintf('./cache/figure_t14_%i_atl%i_%s',iM,atl,AtlNames{atl}));
                    
                    %return
                    h2 = figure('Position',[0 0 600 300],'visible','off');
                    thresh_n_sig = 60;
                    vt = [];
                    vs = [];
                    vn = [];
                    for iswap1 = 1:(length(Adj_plt2)-1)
                        for iswap2 = (iswap1+1):length(Adj_plt2)
                            el1 = CaDefault.Adj_plt2(iswap1,iswap2);
                            el2 = Adj_plt2(iswap1,iswap2);
                            if ( (~isnan(el1)) && (~isnan(el2)) )
                                vt = [vt; el1];
                                vs = [vs; el2];
                                vn = [vn; Adj_plt2_npairs(iswap1,iswap2)];
                            end
                        end
                    end
                    vt = vt(vn >= thresh_n_sig);
                    vs = vs(vn >= thresh_n_sig);
                    frac_diff = (vt - vs)./vt;
                    frac_diff(vt==0) = 0;
                    %frac_diff = (CaDefault.Adj_plt2(Adj_plt2_npairs_sig>thresh_n_sig)-Adj_plt2(Adj_plt2_npairs_sig>thresh_n_sig))./Adj_plt2(Adj_plt2_npairs_sig>thresh_n_sig);
                    histogram(100*frac_diff,'Normalization','pdf');
                    set(gca,'TickDir','out');
                    box off;
                    xlabel('Change in Coherence (%)');
                    ylabel('Probability Density');
                    ax = gca;
                    xlim((max(abs(ax.XLim)))*[(-1) 1]);
                    yticks(linspace(ax.YLim(1),ax.YLim(end),5));
                    print(h2,sprintf('./figures/T14_allatl_bipswap/atl%i_%s/hist_bipswap_difference_metric-%i_thresh-%i_n-%i',atl,AtlNames{atl},iM,thresh_n_sig,length(frac_diff)),'-depsc');
                    close(h2);
                    %histogram((CaDefault.Adj_plt2 - Adj_plt2)./Adj_plt2)
...
%                 end
    ...
    %             xtickangle(90);
        ...
        %         end
                ...
                %
                print(h,sprintf('figures/T14_allatl_bipswap/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss),fig_fmt);
                if (trig_eps)
                    print(h,sprintf('figures/T14_allatl_bipswap/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss),'-depsc');
                end
                ...
                if (iii2 == 1)
                    ...
                    save(sprintf('./brainexport/controls_data_sub-0_freq-%i_atl-%i_bipswap',iM,atl),'S');
                end
...
%                 end
                ...
                if (iii2 == 1)
                    ...
                    fprintf('[*] saved file: %s\n',sprintf('figures/T14_allatl_bipswap/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss));
...
%                 fprintf('[*] Coherence Stdev n: %.2f\n',numel(V_std));
    ...
    %             axis tight;
                ...
                ofname = sprintf('figures/T14_allatl_bipswap/atl%i_%s/Npairs_%s_mag_%i_p_%d_cp_%i_p_%d',atl,AtlNames{atl},metric,...
    ...
    %             axis tight;
                ...
                ofname = sprintf('figures/T14_allatl_bipswap/atl%i_%s/Nsubs_%s_mag_%i_p_%d_cp_%i_p_%d',atl,AtlNames{atl},metric,...
                    round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp);
                ...
                fprintf('[%s] corr number of pairs, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                ofname = sprintf('figures/T14_allatl_bipswap/atl%i_%s/Npairs_%s_magVar_%i_p_%d_cp_%i_p_%d',...
                ...
                fprintf('[%s] corr number of patients, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                ofname = sprintf('figures/T14_allatl_bipswap/atl%i_%s/Nsubs_%s_magVar_%i_p_%d_cp_%i_p_%d',atl,AtlNames{atl},metric,...
                    round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp);
                ofname = replace(ofname,'.','p');
                print(h,ofname,'-depsc');
                ...
                ofname = sprintf('figures/T14_allatl_bipswap/atl%i_%s/mag_%s_magVar_%i_p_%d',atl,AtlNames{atl},metric,...
                ...
                axis tight;
                %return
                ofname = sprintf('figures/T14_allatl_bipswap/atl%i_%s/dist_%s_mag_%i_p_%d',atl,AtlNames{atl},metric,...
                    round(1000*corr_val_cp),corr_pval_cp);
                ofname = replace(ofname,'.','p');
                print(h,ofname,'-depsc');
                close(h);
...
%                 plot(x_bins,n_triu/trapz(x_bins,n_triu),'black--');
                ...
                box off;
                set(gca,'TickDir','out');
                print(h,sprintf('figures/T14_allatl_bipswap/atl%i_%s/intra_r-%i_inter_r-%i_%s_ranksum-%i_ttest-%i',...
