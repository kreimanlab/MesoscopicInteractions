...
elseif contains(host,'kraken')
    dir_art = '/mnt/cuenap2/data/h5_notch20/art_nosz'; %'/media/klab/internal/data/h5_notch20/art';
    dir_res = '/mnt/cuenap2/scripts/opencl/results_res5hz';
    dir_cor = '/mnt/cuenap_ssd/coregistration'; %'/mnt/cuenap_ssd/coregistration';
    dir_h5 = '/mnt/cuenap/data/h5_notch20';
...
%     cp_thresh = all_o(v,3);
    ...
    % Load atlas
    C1 = load(sprintf('%s/%s/label/all_parcellation_150.mat',dir_cor,Subjects{1}));
    ...
    AdjAtl_chans = cell(n_rois,n_rois);
%     AdjAtl_xyz = cell(n_rois,n_rois);
    ...
    for i = 1:length(Subjects)
        ...
        fn_coreg = sprintf('%s/%s/label/all_parcellation_150.mat',dir_cor,sid);
...
%         return
                        ...
                        try
                            for k1 = 1:1%length(i_b1)
                                for k2 = 1:1%length(i_b2)
                                    ...
                                    AdjAtl_chans{ki1,ki2} = [AdjAtl_chans{ki1,ki2},[chan1(k) chan2(k)]];
                                    AdjAtl_chans{ki2,ki1} = [AdjAtl_chans{ki2,ki1},[chan1(k) chan2(k)]];
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_150',sid,metrici),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_150',metrici),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
function save_tmp(sid, R_null)
    save(sprintf('cache/%s_R_null_150',sid),'R_null','-v6');
end
