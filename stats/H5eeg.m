classdef H5eeg
    ...
    methods
        ...
        function tiout = getTimeFromSample(self, stampsDir, sampleNum)
            ...
            lines_t = round(lines_t); % enforce int
            linenum = lines_t;
            
            %fprintf([sprintf('sed -n %i,%ip %s',lines_t,lines_t,stamps_fn),'\n'])
...
%             end
            ...
            lines_times = 0;
            fprintf('[*] First guess: %s, actual: %s\n',datestr(time_guess,'mmmm dd, yyyy HH:MM:SS.FFF AM'),timeStr);
            if (time_guess == time_dn)
                lines_times = linenum;
                fprintf('\tTime accepted.\n')
            else
            ...
            %fprintf('sed -n %i,%ip %s\n',lines_times,lines_times,stamps_fn);
            ...
            fprintf('[*] First guess: %i, actual: %.0f\n',stamp_guess,stamp);
