...
%                 pct_keep = 0.95;
%                 [~,sIdx2] = sort(R1);
%                 R1p(sIdx2(1:n_rem)) = NaN;
%                 R1p(sIdx2((end-n_rem+1):end)) = NaN;
                ...
                set(ax,'TickDir','Out');
                xlabel('Time (hours)');
                xtickv = [t(1):24:t(end),t(end)];
                xtickstr = cell(size(xtickv));
                for ixt = 1:length(xtickv)
                    xtickstr{ixt} = sprintf('%.0f',xtickv(ixt));
                end
                xticks([t(1):24:t(end),t(end)]);
                ...
                if ((nanmean(R1)-6*nanstd(R1)) > ax.YLim(1))
                    ylow = (nanmean(R1)-6*nanstd(R1));
                else
                    ylow = ax.YLim(1);
                end
                if ((nanmean(R1)+6*nanstd(R1)) < ax.YLim(2))
                    yhi = (nanmean(R1)+6*nanstd(R1));
                else
                    yhi = ax.YLim(2);
                end
                ylim([ylow,yhi]);
