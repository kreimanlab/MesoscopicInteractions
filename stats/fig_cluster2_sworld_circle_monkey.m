...
for iM = [1 5] %[1 5]
    ...
    % Load macaque
    CaMa = load('./cache/Adj_plt_mSu_red');
...
%     rois2 = rois2(cov_idx);
    ...
    NodeTable = table(CaMa.Adj_plt_mSu_red_rois,'VariableNames',{'Name'});
    ...
    % circular plot
    col_circ = repmat(0.1*[1 1 1],length(CaMa.Adj_plt_mSu_red_rois),1);
    ...
    axes(ha(1))
    circularGraphJW(Agr,'Label',CaMa.Adj_plt_mSu_red_rois,'Colormap',col_circ); %rois2_col
    axes(ha(2))
    circularGraphJW(A_rand,'Label',CaMa.Adj_plt_mSu_red_rois,'Colormap',col_circ); %rois2_col
    axes(ha(3))
    circularGraphJW(A_lattice,'Label',CaMa.Adj_plt_mSu_red_rois,'Colormap',col_circ); %rois2_col
    ...
    % save
    print(h,sprintf('./figures/fig_cluster2_sworld_circle_monkey_metric-%i',iM),'-depsc');
    print(h,sprintf('./figures/fig_cluster2_sworld_circle_monkey_metric-%i',iM),'-dpng','-r900');
    close(h);
    ...
    Adj_mSu = ME.Adj_plt_mSu_red;
    Adj_mSu_rois = ME.Adj_plt_mSu_red_rois;
    
    n_chans_mSu = length(Adj_mSu);
    AdjV_mSu = nan(nchoosek(n_chans_mSu,2),1);
    cm = 1;
    for ia = 1:(n_chans_mSu-1)
        for ib = (ia+1):n_chans_mSu
            AdjV_mSu(cm) = Adj_mSu(ia,ib);
            cm = cm + 1;
        end
    end
    AdjV = AdjV_mSu(AdjV_mSu>0);
    ...
    print(hh,sprintf('figures/T20_lognormal/metric%i_atl%i_macaque_hist',iM,atl),'-dsvg');
    %saveas(gcf,sprintf('figures/T20_lognormal/metric%i_atl%i_macaque_hist',iM,atl),'epsc');
    close(hh);
    
    %
%     [*] Log(C) kstest p=8.6333e-07, n=479
% 	min:-0.86703422, max:-0.66056529
%     [*] Linear kstest p=1.5901e-08, n=479
% 	min:0.13582064, max:0.21849158
    %return
...
%     end
% 
%     save(sprintf('cache/fig_cluster2_sworld_circle_monkey-%i.mat',iM));
%     
%     %% --------------------------------------------------------------
%     % Weighted SWI
...
%     
%     print(h,sprintf('./figures/fig_cluster2_sworld_circle_monkey_im-%i_swp',iM),'-depsc','-r900');
%     print(h,sprintf('./figures/fig_cluster2_sworld_circle_monkey_im-%i_swp',iM),'-dpng','-r900');
%     close(h);
%     %return
% %     
% %     % Brain metrics
