...
for iM = 1 %1:length(metrics)
    for iSub = 18 %1:length(SubjectsL)
        fprintf('[*] Subject %i\n',iSub)
        for atlM = 2 %1:20
            ...
            for iii = 3 %1:3
                ...
                fn_ca = sprintf('./cache/xsub_out_m00005_%i_atl2.mat',iM);
                Ca = load(fn_ca);
            ...
            %         fprintf('\tSWI = %.6f\n',I);
                ...
                save(sprintf('cache/fig_cluster2_sworld_circle_subjects-%i.mat',iM));
...
%                 Cmk = SC_q{1};
                ...
                for ixvar = 1:length(Xvars)
                    ...
                    T(iSub,ixvar,4) = length(A_input);
                    T(iSub,ixvar,5) = xvar(round((alpha/2)*nsamps));
                    T(iSub,ixvar,6) = xvar(round((1 - (alpha/2))*nsamps));
                end
               % return
...
for ish = 1:n_ish
    TW = squeeze(T(:,:,ish));
    writematrix(TW,'./figures/sworld_circle/output.xlsx','Sheet',ish);
end
