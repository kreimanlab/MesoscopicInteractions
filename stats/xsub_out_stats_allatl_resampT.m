    ...
    for i = 1:length(Subjects)
        ...
        if (~ trig_cache)
            ...
            if (~exist(fn_tmp,'file'))
                ...
                save_tmp(sid,single(R_null),atl,rep);
            else
                try
...
%         return
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_atl%i_resampT-%i',sid,metrici,atl,rep),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_atl%i_resampT-%i',metrici,atl,rep),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
function save_tmp(sid, R_null, atl, rep)
    save(sprintf('cache/%s_R_null_atl%i_resampT-%i',sid,atl,rep),'R_null','-v6');
end
