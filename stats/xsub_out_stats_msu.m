...
if contains(host,'hopperu')
    % Path definitions
    dir_art = '/media/klab/KLAB101/h5_notch20/art_nosz';%'/mnt/cuenap/data/h5_notch20/art_nosz';
...
%     AdjAtl_xyz = cell(n_rois,n_rois);
    ...
    for i = 1:length(Subjects)
        ...
        % Trim anesthesia
        CaA = load('./cache/mSu_artifact_trim.mat');
        art_idx = CaA.msu_art_idx;
        art_idxB = (art_idx ~= 0);
        fprintf('[*] Frac anesthesia: %.6f\n',sum(sum(CaA.msu_art_idx == 6))/numel(CaA.msu_art_idx));
        fprintf('[*] Frac others: %.6f\n',sum(sum((CaA.msu_art_idx ~= 0)&(CaA.msu_art_idx ~= 6)))/numel(CaA.msu_art_idx));
        fprintf('[*] Frac removed: %.6f\n',sum(sum(CaA.msu_art_idx ~= 0))/numel(CaA.msu_art_idx));
