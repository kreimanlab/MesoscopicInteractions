...
for i = 1:n_sub
    ...
    na_tot = na_tot + numel(a);
    
    nai = nai + sum(sum(ai ~= 0));
    Nai = [Nai, sum(sum(ai ~= 0))];
    nai_tot = nai_tot + numel(ai);
    Nai_tot = [Nai_tot, numel(ai)];
...
for i = 1:length(f1(:,1))
    f = f1(i,:);
    ff = f2(i,:);
    fprintf('[%i] tot: %.4f mean: %.4f std: %.4f max: %.4f min: %.4d\n',...
        i,sum(ff)/na_tot,mean(f),std(f),max(f),min(f));
end

cf = Nai./Nai_tot;
fprintf('Coh artifacts: %.4f (%i of %i), mean: %.4f, median: %.4f, std: %.4f, min: %.4f, max: %.4f\n',...
    nai/nai_tot,nai,nai_tot,mean(cf),median(cf),std(cf),min(cf),max(cf));
