...
% dir_art = '/media/jerry/internal/data/h5_notch20/art';%'/media/klab/untitled/h5_notch20/art_nosz'; %'/media/klab/internal/data/h5_notch20/art';
% %dir_res = '/media/klab/untitled/results/coh_w10';
% dir_res = '/media/jerry/internal/data/MesoscopicInteractions-master/opencl/results/w2';
% dir_cor = '/media/klab/internal/data/coreg'; %'/mnt/cuenap_ssd/coregistration';
% dir_h5 = '/media/jerry/internal/data/h5_notch20';
...
%         end
        ...
        % Load 24 hour behavior annotation
        T = load(sprintf('~/Dropbox/24hours/behavior/%s_Annot.mat',sid));
        ...
        % elapsed time offset
        aux1 = double(h5read(ecog.filename,'/h5eeg/aux',[1 1],[1 1]));
        ...
        fn_cache = sprintf('%s/%s-art_idxB-w%i',dir_art,sid,w);
        if (exist([fn_cache,'.mat'],'file'))
            fprintf('[*] Reading art_idxB from: %s\n',fn_cache);
            load(fn_cache);
        else
            fprintf('[*] Re-building artifact definition for w=%i\n',w)
            % resection artifact mark based on W
            artifacts = h5read(fn_art,'/artifacts');
            [~,n_sec] = size(artifacts);
            n_pairs = nchoosek(ecog.n_bchan,2);
            starts = 1:w:n_sec;
            starts = starts(1:(end-1));
            art_idxB = false(n_pairs,length(starts));
            bc = 1;
            for i1 = 1:(ecog.n_bchan-1)
                for i2 = (i1+1):(ecog.n_bchan)
                    cc = 1;
                    for i3 = 1:length(starts)
                        start = starts(i3);
                        stop = start + w - 1;
                        art_idxB(bc,cc) = any([artifacts(i1,start:stop)~=0,artifacts(i2,start:stop)~=0]);
                        cc = cc + 1;
                    end
                    bc = bc + 1;
                end
            end
            save(fn_cache,'art_idxB');
            fprintf('\tDone.\n');
        end
        %return
        ...
        n_beh = length(T.AnnotLabels);
        Pvals = cell(1,n_beh);
        Label_sleep = cell(1,n_beh);
        for j2 = 1:n_beh
            % make raw, adjusting for first elapsed time index and downsampling
            a_on = round((T.Annot_on{j2} - aux1 + 1)./(T.ecog_ds_factors(T.idx_files)));
            a_off = round((T.Annot_off{j2} - aux1 + 1)./(T.ecog_ds_factors(T.idx_files)));
            label_sleep_raw = 2*ones(1,ecog.n_samples);
            for k = 1:length(a_on(:,1))
                label_sleep_raw(a_on(k,1):a_on(k,2)) = 1;
            end
            for k = 1:length(a_off(:,1))
                label_sleep_raw(a_off(k,1):a_off(k,2)) = 0;
            end
            ...
            %-------------------------------------
            %
            w_seconds = w; % seconds used to chunk raw annotations
            w2 = round(ecog.fs)*w_seconds;
            starts = (1:w2:ecog.n_samples);
            starts = starts(1:(end-1));
            label_sleep = nan(size(starts));
            c = 1;
            for j = 1:length(starts)
                idx_start = starts(j);
                idx_end = idx_start + w2 - 1;
                ...
                isvalid = (sum(mode(seg_sleep) == seg_sleep) >= 0.2*length(seg_sleep));
                if (isvalid)
            ...
            for k = 1:n_pairs
                R1 = R(k,:);
                R1(art_idxB(k,:)) = NaN; %remove artifacts
                Rsleep = R1(label_sleep == 1);
                Rwake = R1(label_sleep == 0);
                ...
                pvals(k) = p;
                if (k == n_pairs)
                    fprintf('[%i/%i %s] %i of %i, beh %i of %i\n',i,length(SubjectsSleep),sid,k,n_pairs,j2,n_beh);
                end
                if (trig_permutation)
            ...
            Pvals{j2} = pvals;
            Label_sleep{j2} = label_sleep;
        
        end
        
        save(sprintf('./cache/%s_24h_metric-%i_atl-%i',sid,metrici,atl),'-v6','-regexp','^(?!(CaSleep|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
        %return
...
%                                     
%                                     AdjAtlCT{ki1,ki2} = [AdjAtlCT{ki1,ki2}; elCT];
%                                     AdjAtlCT{ki2,ki1} = [AdjAtlCT{ki2,ki1}; elCT];
% 
