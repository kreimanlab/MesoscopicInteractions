...
dir_artLpp = '/media/jerry/KLAB101/h5_notch20/art_nosz';
dir_resLpp = '/media/jerry/KLAB101/results/coh_w10';
...
% Coh thresh - 0.172
% Good examples, and their coherence
% .987 - 0.364
% .920 - 0.280
% .996 - 0.535
...
roi_1 = 'superiortemporal'; %'parstriangularis';
roi_2 = 'parsopercularis'; %'inferiorparietal';
...
%             has_roi_r = strcmp(atl_labels{ii1},roi_2) & strcmp(atl_labels{ii2},roi_1);
            ...
            if (((has_roi_f || has_roi_r) && (Dmat(ii1,ii2) > dist_thresh)) && cond_bpair_bypass)
                pname = sprintf('%s__%i_%s_%s__%i_%s_%s__%imm_ct%i_mag%i_coh%it%i_str%i',...
                    ...
                    iMpp,round(1000*coh_thresh(count)),round(1000*strength));
                %fprintf('%s\n',pname);
...
%                         box off;
                        ...
                        print(h,sprintf('figures/T1d2/%s',pname),fig_fmt);
                        if (trig_eps)
                            print(h,sprintf('figures/T1d2/%s',pname),'-depsc');
                        end
                        close(h);
                        ...
                        if (trig_brain)
                            ...
                            print(h,sprintf('figures/T1d2/%s_brain',pname),fig_fmt);
                            if (trig_eps)
                                print(h,sprintf('figures/T1d2/%s_brain',pname),'-depsc');
                            end
                            close(h);
                        end
