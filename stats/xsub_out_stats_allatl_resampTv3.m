...
%     dir_cor = '/media/klab/KLAB101/coreg'; %'/media/klab/internal/data/coreg'; %'/mnt/cuenap_ssd/coregistration';
%     dir_h5 = '/media/klab/KLAB101/h5_notch20';
    ...
    dir_cor = '/media/klab/untitled/coreg'; %'/media/klab/internal/data/coreg'; %'/mnt/cuenap_ssd/coregistration';
...
elseif contains(host,'Leibniz')
    ...
    dir_cor = '/Volumes/KLAB101/coreg'; %'/media/klab/internal/data/coreg'; %'/mnt/cuenap_ssd/coregistration';
...
%nperm = 1;
%for iperm = (1:100) %1:nperm
%for iperm = (13:100) %1:nperm
for iperm = 1:1000

rep_sav = cell(1,length(Subjects));
...
for metrici = [1 5] %1:length(metrics) %1:length(metrics)
...
%     AdjAtl_xyz = cell(n_rois,n_rois);
    ...
    %for i = 45
        ...
        % Resample by shrinking dataset 
        [~,n_R] = size(R);
        
        if (rep == 1)
            
            n_R_half_bool = false(1,n_R);
            n_R_half_seq = 1:n_R;
            %n_R_half = (randperm(n_R));
            
            % shrink factor
            %n_R_i = round(n_R / 24);
            n_R_i = 60; % 10 minutes
            
            % Carve from beginning
            %n_R_half_bool( n_R_half_seq(1:n_R_i) ) = true;
            
            % Carve from middle
            %n_R_mid = floor(n_R/2);
            
            % Carve from random
            n_R_mid = randi([1 (n_R - n_R_i + 1)]);
            
            n_R_half_bool( n_R_half_seq((n_R_mid):(n_R_mid + n_R_i-1)) ) = true;
            
            n_R_half = n_R_half_bool;
            n_R_half_not = ~n_R_half_bool; %n_R_half_seq((n_R_i+1):end);
            rep_sav{i} = n_R_half;
            %seg_sec = (60*60)/w_art;
            
            %return
            %art_idxB(:,1:n_R_half) = true;
            art_idxB(:,n_R_half_not) = true; % set majority to art
        elseif (rep == 2)
            n_R_half = rep_sav{i};
            %art_idxB(:,(n_R_half+1):end) = true;
            art_idxB(:,n_R_half) = true; % set minority to art
        
%         % Second segmentation method
%         elseif (rep == 3)
%             % Randomly set segments of <seg_sec> length to artifacts
%             seg_sec = (60*60)/w_art; % number of seconds to separate by
%             [~,nAi] = size(art_idxB);
%             n_stride = ceil(nAi/seg_sec);
%             
%             % Make leave-out decision vector
%             stat = false(1,n_stride);
%             stat(1:round(n_stride/2)) = true;
%             stat = stat(randperm(n_stride));
%             
%             % leave-out index
%             idxRem = false(1,nAi);
%             for ir = 1:n_stride
%                 i_start = (ir-1)*seg_sec + 1;
%                 i_end = i_start + seg_sec - 1;
%                 if (i_end > nAi)
%                     i_end = nAi;
%                 end
%                 idxRem(i_start:i_end) = stat(ir);
%             end
%             
%             % Apply selection
%             art_idxB(:,idxRem) = true;
%         elseif (rep == 4)
%             % Apply selection
%             art_idxB(:,(~idxRem)) = true;
%             return
        ...
        if (~ trig_cache)
            ...
            if (~exist(fn_tmp,'file'))
                ...
                save_tmp(sid,single(R_null),atl,rep,iperm);
            else
                try
...
%         return
            ...
            for k = 1:ecog.n_comb
                if (d_i(k)) % only map if distance threshold is met
                    ...
                    if (~isSameHemi)
                        ...
                        % ROI index for one bipolar from two physical
                        i_b1 = [i_b1c1,i_b1c2];
                        i_b2 = [i_b2c1,i_b2c2];
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_atl%i_resampTv3-%i-%i',sid,metrici,atl,rep,iperm),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_atl%i_resampTv3-%i-%i',metrici,atl,rep,iperm),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
fprintf('All Reps Done.\n')

end
fprintf('All Perms Done.\n')


function save_tmp(sid, R_null, atl, rep, iperm)
    save(sprintf('cache/%s_R_null_atl%i_resampTv3-%i-%i',sid,atl,rep,iperm),'R_null','-v6');
end
