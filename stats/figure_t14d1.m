...
for iM = 1:length(metrics)
    for iSub = 1:length(SubjectsL)
        ...
        sid_sav = sid;
        iSub_sav = iSub;
        load(sprintf('%s/xsub_out_all_%i_atl2.mat',dir_cacheL,iM));
        sid = sid_sav;
        iSub = iSub_sav;
...
%         end
        ...
        colormap(map);
        cbpos = cb.Position;
%         if (strcmp(sid,'sub48') || strcmp(sid,'sub9') || strcmp(sid,'sub19') || strcmp(sid,'sub25') || strcmp(sid,'sub34')|| strcmp(sid,'sub42'))
%             cbpos(2) = 2*cbpos(2);
%             cbpos(4) = 0.5*cbpos(4);
%         else
%             cbpos(2) = 2.6*cbpos(2);
...
%             end
        ...
        sid = 'mX';
        ofi = fopen(sprintf('figures/T14d1/Figure_W2-%iB_Adj_%s_%s_%s_sig-%i-of-%i.txt',...
            sid_int,metric,sid,ss,n_sig2,(n_sig2 + n_nosig2)),'w');
        fprintf(ofi,'%s',legend_txt);
        fclose(ofi);
        
        % ---------------------
        
        print(h,sprintf('figures/T14d1/Figure_W2-%iB_Adj_%s_%s_%s_sig-%i-of-%i',...
            sid_int,metric,sid,ss,n_sig2,(n_sig2 + n_nosig2)),fig_fmt,'-r500');
        if (trig_eps)
            %print(h,sprintf('figures/T14d1/Figure_W2-%iB_Adj_%s_%s_%s',sid_int,metric,sid,ss),'-depsc');
            print(h,sprintf('figures/T14d1/Figure_W2-%iB_Adj_%s_%s_%s_sig-%i-of-%i',...
                sid_int,metric,sid,ss,n_sig2,(n_sig2 + n_nosig2)),'-depsc','-r500');
        end
