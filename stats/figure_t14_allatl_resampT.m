...
system('mkdir figures/T14_allatl_resampT');
...
for atl = [2] %1:20
    ...
    system(sprintf('mkdir figures/T14_allatl_resampT/atl%i_%s',atl,AtlNames{atl}));
    ...
    for iM = [1] %1:length(metrics) % [1 5] %
        ...
        for repL = [1 2]
            % Load human cache
            Ca_hum = load(sprintf('%s/xsub_out_all_%i_atl%i_resampT-%i.mat',dir_cacheL,iM,atl,repL));
    ...
    %adjct_dist_cl = adjct_dist(ind_hum2mac,ind_hum2mac);
                ...
                %                 yc = yc + 1;
                        ...
                        save(sprintf('./cache/figure_t14_%i_atl%i_%s_resampT_rep%i',iM,atl,AtlNames{atl},repL),'AdjMag','AdjMagVar','Adj_plt','Adj_plt2','Adj_plt2_cl','Y','Z','M','cluster_i','rois_plt','atl');
    ...
    %                 end
        ...
        %             xtickangle(90);
            ...
            %         end
                    ...
                    %
                    print(h,sprintf('figures/T14_allatl_resampT/atl%i_%s/Adj_%s_%s_rep%i',atl,AtlNames{atl},metric,ss,repL),fig_fmt);
                    if (trig_eps)
                        print(h,sprintf('figures/T14_allatl_resampT/atl%i_%s/Adj_%s_%s_rep%i',atl,AtlNames{atl},metric,ss,repL),'-depsc');
                    end
...
%                     end
    ...
    %                     end
    %                     axis off;
    %                     print(h,sprintf('figures/T14_allatl_resampT/atl%i_%s/Adj_%s_%s_dendro',atl,AtlNames{atl},metric,ss),fig_fmt);
    %                     if (trig_eps)
    %                         print(h,sprintf('figures/T14_allatl_resampT/atl%i_%s/Adj_%s_%s_dendro',atl,AtlNames{atl},metric,ss),'-depsc');
    %                     end
    %                     close(h);
    %                 end
                    ...
                    if (iii2 == 1)
                        ...
                        fprintf('[*] saved file: %s\n',sprintf('figures/T14_allatl_resampT/atl%i_%s/Adj_%s_%s',atl,AtlNames{atl},metric,ss));
    ...
    %                 fprintf('[*] Coherence Stdev n: %.2f\n',numel(V_std));
        ...
        %             axis tight;
                    ...
                    ofname = sprintf('figures/T14_allatl_resampT/atl%i_%s/Npairs_%s_mag_%i_p_%d_cp_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
        ...
        %             axis tight;
                    ...
                    ofname = sprintf('figures/T14_allatl_resampT/atl%i_%s/Nsubs_%s_mag_%i_p_%d_cp_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                        round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp,repL);
                    ...
                    fprintf('[%s] corr number of pairs, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                    ofname = sprintf('figures/T14_allatl_resampT/atl%i_%s/Npairs_%s_magVar_%i_p_%d_cp_%i_p_%d_rep%i',...
                    ...
                    fprintf('[%s] corr number of patients, cp: %.5f (p = %.5f), n=%i\n',metric(3:end),corr_val_cp,corr_pval_cp,length(CP(is_sig_mag)));
                    ofname = sprintf('figures/T14_allatl_resampT/atl%i_%s/Nsubs_%s_magVar_%i_p_%d_cp_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                        round(1000*corr_val_mag),corr_pval_mag,round(1000*corr_val_cp),corr_pval_cp,repL);
                    ofname = replace(ofname,'.','p');
                    print(h,ofname,'-depsc');
                    ...
                    ofname = sprintf('figures/T14_allatl_resampT/atl%i_%s/mag_%s_magVar_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                    ...
                    axis tight;
                    %return
                    ofname = sprintf('figures/T14_allatl_resampT/atl%i_%s/dist_%s_mag_%i_p_%d_rep%i',atl,AtlNames{atl},metric,...
                        round(1000*corr_val_cp),corr_pval_cp,repL);
                    ofname = replace(ofname,'.','p');
                    print(h,ofname,'-depsc');
                    close(h);
                    ...
                    box off;
                    set(gca,'TickDir','out');
                    print(h,sprintf('figures/T14_allatl_resampT/atl%i_%s/intra_inter_%s_ranksum-%i_ttest-%i_rep%i',atl,AtlNames{atl},metric,...
        ...
        AdjMagRep1 = AdjMagRep{1};
        AdjMagRep1 = AdjMagRep1(known_idx,known_idx);
        AdjMagRep2 = AdjMagRep{2};
