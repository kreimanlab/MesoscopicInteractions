...
for i = 1:length(metricsp)
    ...
    % artifical cluster create
    ztmp = cl_adj < 0;
%     for itmp = 1:40
%         cl_adj(itmp+2,:) = 0.8 * cl_adj(2,:) + 0.2*rand(1,length(cl_adj));
%     end
    cl_adj(ztmp) = -1;
    %return
    ...
    %======================================================================
    % k-means clustering
    fn_kmeans = sprintf('cache/figure_t17_%i_kmedoids_fillzeros',i);
    ...
    if (exist([fn_kmeans,'.mat'],'file'))
        ...
        for ki = 1:length(K)
            ...
            [idx,C,sumd,D] = kmedoids(A2km,k,'distance',@nanDist4clustering_neg,'Options',options,'OnlinePhase','on','Replicates',4*12);
            Res{ki,1} = idx;
            Res{ki,2} = C;
            Res{ki,3} = sumd;
            Res{ki,4} = D;
...
%             end
        ...
        print(h,sprintf('figures/T17/kmeans-%i_%s_%s_nosig-%i_fillzeros',K(k),metric(3:end),SURFACE_TYPE,trig_blot_nosig),'-depsc');
        print(h,sprintf('figures/T17/kmeans-%i_%s_%s_nosig-%i_fillzeros',K(k),metric(3:end),SURFACE_TYPE,trig_blot_nosig),'-dpng','-r400');
        close(h);
        ...
        print(h,sprintf('figures/T17/kmeans-%i_%s_%s_nosig-%i_tsne_fillzeros',K(k),metric(3:end),SURFACE_TYPE,trig_blot_nosig),'-depsc','-r400');
        close(h);
    end
