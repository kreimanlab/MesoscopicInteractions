% All interactions with 58 (pars tri), 30 seconds

close all;
clear;

dir_artL = '/media/jerry/data/h5_notch20/art_nosz';
dir_resL = '/home/jerry/data/results/coh_w10';
...
%     end
    ...
    w_times = round((60*60)/w_coh); % 60 minutes width
    w_toffset = round((5*60)/w_coh); % 5 minute offset to the past
    [n_bchan,~] = size(ecog.bip);
    n_comb = nchoosek(n_bchan,2);
    R = h5read(fn_graph,'/R',[1 w_idx-w_toffset],[n_comb w_times]);
    chan1 = Ca.chan1 + 1;
    chan2 = Ca.chan2 + 1;
...
%     R_passt = R > Ca.coh_thresh;
%     R2 = R;
%     R2(~R_passt) = NaN;
%     R3 = nanmean(R2,2);
%     sig_i = (Ca.Dmats > Ca.dist_thresh) & (R3 > Ca.coh_thresh); % Ca.mag
%     mag = double(R3(sig_i,:));
%     
    sig_i = (Ca.is_x ~= 0);
    mag = double(Ca.mag(sig_i,:));
    ...
    for j = 1:length(chan1)
        ...
        if (true) %( (b1 == bchan2_const) || (b2 == bchan2_const) )
    ...
    system('mkdir figures/T011d6');
    oname = sprintf('figures/T011d6/fsaverage_sym_remap_avgMM-%i_medianMM-%i',...
