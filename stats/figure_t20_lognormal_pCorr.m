...
for atl = [2] %1:20
    ...
    for iM = 1 %1:5 %1:length(metrics) % [1 5] %
        ...
        % Load human cache
        Ca_hum = load(sprintf('%s/xsub_out_all_%i_atl%i_pCorr.mat',dir_cacheL,iM,atl));
...
%         return
        ...
        for i1 = 1:(n_rois-1)
            for i2 = (i1+1):n_rois
                if ((i1 ~= 1) && (i2 ~= 1)) % filter out unknown
                    L = AdjMagL{i1,i2};
        ...
        box off;
        set(gca,'TickDir','out');
        print(h,sprintf('figures/T20_lognormal/pCorr_metric%i_atl%i_mag_vs_ct_r-%i_p-%i',iM,atl,round(r*1000),round(pval*1000)),'-depsc');
        print(h,sprintf('figures/T20_lognormal/pCorr_metric%i_atl%i_mag_vs_ct_r-%i_p-%i',iM,atl,round(r*1000),round(pval*1000)),'-dpng');
        close(h);
        ...
        %return
        print(hh,sprintf('figures/T20_lognormal/pCorr_metric%i_atl%i_hist',iM,atl),'-dsvg');
...
%                     end
                ...
                print(h,sprintf('figures/T20_lognormal/pCorr_metric%i_atl%i_%s',iM,atl,rois{ir}),'-depsc');
                print(h,sprintf('figures/T20_lognormal/pCorr_metric%i_atl%i_%s',iM,atl,rois{ir}),'-dpng');
                close(h);
...
for iM = [1]
    ...
    % [*] Log(C) kstest p=1.4780e-01, n=183
    %return
    print(hh,sprintf('figures/T20_lognormal/pCorr_150_metric%i_atl%i_hist',iM,atl),'-dsvg');
    %saveas(gcf,sprintf('figures/T20_lognormal/metric%i_atl%i_hist',iM,atl),'epsc');
    close(hh);
end
...
% [*] Log(C) kstest p=7.0618e-01, n=182
% 	min:-0.43399366, max:-0.20439063
% [*] Linear kstest p=3.5855e-01, n=182
% 	min:0.36813435, max:0.62461063
% [fusiform] n_bip_pairs: 1825
% [inferiorparietal] n_bip_pairs: 947
% [inferiortemporal] n_bip_pairs: 3302
% [lateralorbitofrontal] n_bip_pairs: 638
% [middletemporal] n_bip_pairs: 3644
% [parsopercularis] n_bip_pairs: 395
% [parsorbitalis] n_bip_pairs: 461
% [parstriangularis] n_bip_pairs: 350
% [postcentral] n_bip_pairs: 522
% [precentral] n_bip_pairs: 507
% [rostralmiddlefrontal] n_bip_pairs: 1175
% [superiorfrontal] n_bip_pairs: 619
% [superiorparietal] n_bip_pairs: 292
% [superiortemporal] n_bip_pairs: 1965
% [supramarginal] n_bip_pairs: 1199
