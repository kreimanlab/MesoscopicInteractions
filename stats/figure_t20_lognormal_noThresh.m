...
for atl = [2] %1:20
    ...
    for iM = 5 %1:5 %1:length(metrics) % [1 5] %
        ...
        % Load human cache
        Ca_hum = load(sprintf('%s/xsub_out_all_%i_atl%i_noCt.mat',dir_cacheL,iM,atl));
...
%         return
        ...
        box off;
        set(gca,'TickDir','out');
        print(h,sprintf('figures/T20_lognormal/noThresh_metric%i_atl%i_mag_vs_ct_r-%i_p-%i',iM,atl,round(r*1000),round(pval*1000)),'-depsc');
        print(h,sprintf('figures/T20_lognormal/noThresh_metric%i_atl%i_mag_vs_ct_r-%i_p-%i',iM,atl,round(r*1000),round(pval*1000)),'-dpng');
        close(h);
        ...
        %return
        print(hh,sprintf('figures/T20_lognormal/noThresh_metric%i_atl%i_hist',iM,atl),'-dsvg');
...
%                     end
                ...
                print(h,sprintf('figures/T20_lognormal/noThresh_metric%i_atl%i_%s',iM,atl,rois{ir}),'-depsc');
                print(h,sprintf('figures/T20_lognormal/noThresh_metric%i_atl%i_%s',iM,atl,rois{ir}),'-dpng');
                close(h);
...
%     plot(x_cen,y_norm2,':','color',cc(1,:));
    ...
    % [*] Log(C) kstest p=1.4780e-01, n=183
    %return
    print(hh,sprintf('figures/T20_lognormal/noThresh_150_metric%i_atl%i_hist',iM,atl),'-dsvg');
    %saveas(gcf,sprintf('figures/T20_lognormal/metric%i_atl%i_hist',iM,atl),'epsc');
    close(hh);
end
