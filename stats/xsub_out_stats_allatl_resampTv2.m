...
for rep = [3 4] %[3 4]
    % Note: reps 5 and 6 separated data into 4-hour epochs, equal amounts
...
for metrici = [5] %1:length(metrics) %1:length(metrics)
...
%         return
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_atl%i_resampTv2-%i-%i',sid,metrici,atl,rep,iperm),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_atl%i_resampTv2-%i-%i',metrici,atl,rep,iperm),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
function save_tmp(sid, R_null, atl, rep, iperm)
    save(sprintf('cache/%s_R_null_atl%i_resampTv2-%i-%i',sid,atl,rep,iperm),'R_null','-v6');
end
