...
elseif contains(host,'kraken')
    ...
    dir_res = '/media/klab/untitled/results_p';
...
%     dir_cor = '/mnt/cuenap_ssd/coregistration';
%     dir_h5 = '/mnt/cuenap/data/h5_notch20';
    dir_art = '/nas_share/cuenap/data/h5_notch20_orig/art_nosz2';
    dir_res = '/nas_share/cuenap/data/opencl/results_p';
    dir_cor = '/home/admin/Desktop/NAS/cuenap_ssd/coregistration';
    dir_h5 = '/nas_share/cuenap/data/h5_notch20_orig';
end
...
for metrici = [1]%1:length(metrics) %1:length(metrics)
...
%     AdjAtl_xyz = cell(n_rois,n_rois);
    ...
    for i = 1:length(Subjects)
        ...
        R = abs(h5read(fn_graph,'/R',[1 1],size(art_idx)));
...
%         return
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_atl%i_pCorr',sid,metrici,atl),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_atl%i_pCorr',metrici,atl),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
function save_tmp(sid, R_null, atl, metrici)
    save(sprintf('cache/%s_R_null_atl%i_%i_pCorr',sid,atl,metrici),'R_null','-v6');
end
