...
elseif contains(host,'kraken')
    dir_art = '/media/jerry/internal/data/h5_notch20/art_nosz'; %'/media/klab/internal/data/h5_notch20/art';
    dir_res = '/home/jerry/data/opencl2/results_res5hz_w10_gw';
...
for i = 1:length(cfiles)
    cfiles{i} = sprintf('cache/%s_R_null_gw.mat',Subjects{i});
end
...
%     AdjAtl_xyz = cell(n_rois,n_rois);
    ...
    for i = 1:length(Subjects)
        ...
        if (~ trig_cache)
            ...
            fn_tmp = sprintf('%s/cache/%s_R_null_gw.mat',curr_d,sid);
...
%         return
    ...
    %                                 AdjAtl_xyz{ki2,ki1} = [AdjAtl_xyz{ki2,ki1}; ecog.bip()];
        ...
        save(sprintf('cache/xsub_out_%s_%i_atl%i_gw',sid,metrici,atl),'-v6','-regexp','^(?!(AdjAtl|AdjAtlN|AdjAtl_sid|R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
    ...
    % Save final result
    save(sprintf('cache/xsub_out_all_%i_atl%i_gw',metrici,atl),'-v6','-regexp','^(?!(R|R_null|R4m|R4mN|D|d_i|art_idx|art_idxB)$).');
...
function save_tmp(sid, R_null, atl)
    save(sprintf('cache/%s_R_null_atl%i_gw',sid,atl),'R_null','-v6');
end
